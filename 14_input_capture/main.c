...
// ------ Timer 3 (TIM3)
#define TIM3_OFFSET			  (0x0400UL)
#define TIM3_BASE             (APB1_PERIPHERAL_BASE + TIM3_OFFSET)
// ------- --------- --------- --------- --------- --------- --------- --------
#define BIT_0                   (1U << 0)
#define BIT_1                   (1U << 1)
#define BIT_2                   (1U << 2)
...
#define SR_CC1IF				BIT_1         // RM, 13.4.5, bit 1, flag
#define CCER_CC1E               BIT_0         // Capture/Compare 1 output en.
#define CCER_CC1S               BIT_0 		  // Capture/Compare CC1 ch input.
#define AFR5_TIM                BIT_20        // AFRL AF1 0001 bits 20-23.
#define AFR6_TIM				BIT_25        // AFRL AF2 0010 bits 24-27.
// ------- --------- --------- --------- --------- --------- --------- --------
...
int main(void)
{
	// MCU: Connect the jumper wire from PA5 to PA5.
	tim2_pa5_output_compare();  // the pin is toggled every second (source)
	tim3_pa6_input_capture();   // captures the source

	while (1)
	{
		// Wait before the edge has been captured.
...
void tim3_pa6_input_capture(void)
{
	// 1. Enable the RCC clock access to GPIOA.
	RCC->AHB1ENR |= GPIOAEN_BIT;

	// 2. Configure PA6 mode to alternate function.
	// RM0383-STM32F411XCE-REFERENCE-MANUAL.PDF
	// 8.4.1 GPIO port mode register (GPIOx_MODER).
	// Bits 12 and 13 MODER6[1:0].
	// 10: Alternate function mode.
	GPIOA->MODER &= ~BIT_12;   // 0
	GPIOA->MODER |= BIT_13;    // 1

	// 3. Configure the PA6  to alternate function type.
	// STM32F411CE_DATASHEET.PDF, Table 9. Alternate function mapping.
	// (PA6, AF02) : TIM3_CH1
	// RM0383-STM32F411XCE-REFERENCE-MANUAL.PDF
	// 8.4.9 GPIO alternate function low register (GPIOx_AFRL).
	// 0010: AF2
	GPIOA->AFR[0] |= AFR6_TIM;  // 0010 at bits 24-27, set bit 25
	...
	// Configured as input: 0: capture disabled, 1: capture enabled.
	TIM3->CCER  = CCER_CC1E;

	// 8. Enable TIM3.
	// RM0383-STM32F411XCE-REFERENCE-MANUAL.PDF
	// 13.4.1 TIMx control register 1 (TIMx_CR1).
	// Bit 0 CEN: Counter enable. 0: Counter diabled, 1 Counter enabled.
	TIM3->CR1 = CR1_CEN;
}
...
void tim2_pa5_output_compare(void)
{
	// 0a. Enable clock access to GPIOA.
	RCC->AHB1ENR |= GPIOAEN_BIT;

	// 0b. Set PA5 mode to alternate function.
	// RM0383-STM32F411XCE-REFERENCE-MANUAL.PDF
	// GPIO port mode register (GPIOx_MODER), MODER5[1:0] bits 10 and 11.
	// 10: Alternate function mode.
	GPIOA->MODER &= ~BIT_10;      // 0
	GPIOA->MODER |= BIT_11;       // 1

	// 0c. Set PA5 alternate function to TIM2_CH1.
	// STM32F411CE_DATASHEET.PDF, Table 9. Alternate function mapping
	// (PA5, AF01), PA5 is also TIM2_CH1 / TIM2_ETR.
	// RM0383-STM32F411XCE-REFERENCE-MANUAL.PDF
	// 8.4.9 GPIO alternate function low register (GPIOx_AFRL).
	// AFRL5[3:0], bits 20 - 23. AFRLy selection 0001: AF1.
	...
	// 4a. Set output compare toggle mode.
	// RM0383-STM32F411XCE-REFERENCE-MANUAL.PDF
	// 12.4.7 TIM1 capture/compare mode register 1 (TIMx_CCMR1).
	// Bits 6:4 OC1M: Output Compare 1 mode
	// The register reset value is 0x0000.
	// 011: Toggle - OC1REF toggles when TIMx_CNT = TIMx_CCR1.
	TIM2->CCMR1 = (BIT_4 | BIT_5);

	// 4b. Enable timer 2, channel 1 in compare mode.
    // 12.4.9. TIM1 capture/compare enable register (TIMx_CCER)
	// Bit 0 CC1E: Capture/Compare 1 output enable.
	// 0: Capture disabled, 1: Capture enabled.
	TIM2->CCER |= CCER_CC1E;

	// 5. Clear up the counter register.
	TIM2->CNT = 0;
